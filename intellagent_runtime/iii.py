"""III — WiseOrder v0.2.0 canonical hash function.

The math underneath is WiseDigest-3, defined and reference-implemented in:

  - ~/Desktop/wop/research/WiseDigest-3.md   (normative spec, 230 lines)
  - ~/Desktop/wop/src/wise/digest_v3.py      (reference Python implementation)

Per WO-018 design directives:
  - "USE DIGEST 3"
  - "NO DONT CHANGE NAMES OR ANYTHING LEAVE IT OG"
  - "MAKE IT ME FOR III"
  - "NOT AT ALL WHERE ITS SAYS WD3 NO III"

This file is a byte-for-byte copy of the WOP reference (constants, math, class
name `WiseDigest3`, domain separator `b"WiseDigest-3"` — all left OG). The
WOP-side module remains the normative source. WiseOrder code paths reference
the algorithm under the name **III**, which is exposed as a module-level
alias at the bottom of this file.

What this means on the chain:
  - The algorithm string in every v0.2.0 manifest is "III".
  - The hash bytes produced are mathematically identical to WOP's WiseDigest-3.
  - A third-party verifier can run WOP's reference and confirm.

The WOP source MUST NOT be modified. If WOP ever ships a new version of
WiseDigest-3, this file is updated in lockstep — never the other way.
"""
...
# -- WiseOrder-side label --------------------------------------------------
# The WiseOrder v0.2.0 chain refers to this hash as "III". The math is
# WiseDigest-3 from WOP, unchanged. Anywhere in WiseOrder code that wants to
# stamp a hash with the canonical v0.2.0 algorithm, import III and call it.

III = WiseDigest3
III_NAME = "III"


def iii(data: bytes) -> str:
    """Compute III (= WiseDigest-3) hex digest over `data`."""
